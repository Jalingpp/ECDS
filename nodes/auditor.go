...
type Auditor struct {
	...
	// ACServer                  *grpc.Server                 //用于服务客户端的Server对象
...
// 	pb.UnimplementedACServiceServer // 嵌入未实现的 gRPC 服务器结构体
// }
...
func NewAuditor(ipaddr string, snaddrfilename string, dn int, pn int) *Auditor {
	...
	auditor := &Auditor{snaddrmap, cfdsmap, cfpsmap, metaFileMap, dn, pn, pcfdsmap, pcfpsmap, pb.UnimplementedACServiceServer{}}
...
func (ac *Auditor) SelectSNs(ctx context.Context, sreq *pb.StorageRequest) (*pb.StorageResponse, error) {
	log.Printf("Received message from client: %s", sreq.ClientId)
	// 为该客户端文件随机选择存储节点
...
func (ac *Auditor) SelectStorNodes(filename string) ([]string, []string) {
	...
	for i := 1; i < ac.DataNum+ac.ParityNum+1; i++ {
		if selectednum[strconv.Itoa(i)] {
			dssnaddrs = append(dssnaddrs, ac.SNAddrMap["sn"+strconv.Itoa(i)])
		} else {
			pssnaddrs = append(pssnaddrs, ac.SNAddrMap["sn"+strconv.Itoa(i)])
		}
	}
