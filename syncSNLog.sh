...
REMOTE_LOG_FILE="/root/DSN/ECDS/data/output_sn.log"

# 定义本地日志文件路径
# LOCAL_LOG_FILE="/home/ubuntu/ECDS/data/output_sn.log"
LOCAL_LOG_FILE="/root/DSN/ECDS/data/output_sn.log"
...
# while true; do
    ...
    do
        ...
        if [[ $ip_addr =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
            ...
            rsync -avz -e "sshpass -p $SSH_PASSWORD ssh -o StrictHostKeyChecking=no" "root@$ip_addr:$REMOTE_LOG_FILE" "$LOCAL_LOG_FILE.tmp" && cat "$LOCAL_LOG_FILE.tmp" >> "$LOCAL_LOG_FILE" && rm "$LOCAL_LOG_FILE.tmp"
