...
REMOTE_LOG_FILE="/root/DSN/ECDS/data/output_client.log"

# 定义本地日志文件路径
# LOCAL_LOG_FILE="/home/ubuntu/ECDS/data/output_client.log"
LOCAL_LOG_FILE="/root/DSN/ECDS/data/output_client.log"
...
rsync -avz -e "sshpass -p $SSH_PASSWORD ssh -o StrictHostKeyChecking=no" "root@$CLIENT_IP:$REMOTE_LOG_FILE" "$LOCAL_LOG_FILE.tmp" && cat "$LOCAL_LOG_FILE.tmp" >> "$LOCAL_LOG_FILE" && rm "$LOCAL_LOG_FILE.tmp"
